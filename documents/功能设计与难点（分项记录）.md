...
## 路由与状态管理（AppRoute + ViewModel）
- 通俗说明
  - 以前页面跳转靠一堆开关变量，容易乱；现在用一个“导航员”（ViewModel）统一管理，要去哪直接告诉它。
- 整体设计
  - `AppRoute` Sealed Class 定义所有页面（首页、相册、编辑、拼图等）及其参数；`HomeViewModel` 持有 `StateFlow<AppRoute>`；`HomeScreen` 根据流状态渲染对应页面。
- 开发遇到的困难
  - `HomeScreen` 中 `rememberSaveable` 变量过多（192行+），逻辑分散，难以维护和扩展。
- 解决思路
  - 引入 MVI 思想，将导航状态提升至 ViewModel，UI 变为无状态的纯渲染组件。
- 关键代码路径
  - `app/src/main/java/com/example/xingtuclone/ui/navigation/AppRoute.kt`（路由定义）
  - `app/src/main/java/com/example/xingtuclone/ui/HomeViewModel.kt`（状态管理）
  - `app/src/main/java/com/example/xingtuclone/ui/HomeScreen.kt`（响应式 UI）

## 统一导出（Exporter）
- 通俗说明
  - 不管是拼图、修图还是消除，保存照片都走同一个通道，保证都能存进相册且体验一致。
- 整体设计
  - `Exporter` 单例对象封装 `saveBitmapToGalleryReturnUri`，提供挂起函数 `exportBitmap` 和 `exportBitmaps`。
- 解决思路
  - 消除各页面重复的保存代码，统一处理上下文与 IO 异常。
- 关键代码路径
  - `app/src/main/java/com/example/xingtuclone/ui/utils/Exporter.kt`
