...
## 架构重构（路由与导出统一）
- 困难
  - `HomeScreen` 状态爆炸：大量布尔值控制页面显隐，导航逻辑分散，难以维护。
  - 导出逻辑冗余：拼图、批量修图、消除等页面各自实现保存逻辑，导致 Bug（如批量修图无法退出、消除页保存失效）。
- 解决方案
  - **统一路由**：引入 `AppRoute` Sealed Class 与 `HomeViewModel`，使用 `StateFlow` 管理当前页面状态，实现单向数据流。
    - 路由定义：`app/src/main/java/com/example/xingtuclone/ui/navigation/AppRoute.kt`
    - 状态管理：`app/src/main/java/com/example/xingtuclone/ui/HomeViewModel.kt`
  - **统一导出**：封装 `Exporter` 单例，统一处理 Bitmap 保存与 `Uri` 返回。
    - 导出工具：`app/src/main/java/com/example/xingtuclone/ui/utils/Exporter.kt`
